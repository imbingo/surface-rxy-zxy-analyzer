...
class UITests(unittest.TestCase):
    ...
    def test_progress_cancel_and_commit_boundary(self):
        ...
        self.assertIn('导入已取消', d.windowTitle())
        d.close()
        d = ImportProgressDialog('wrong.dat')
        d.set_progress(8, '正在检查格式')
        d.finish(False, '导入失败：当前文件与所选导入策略不匹配。')
        self.assertIn('导入失败', d.windowTitle())
        self.assertIn('⛔ 导入失败', d.detail.text())
        self.assertIn('border:2px solid', d.detail.styleSheet())
        self.assertEqual(d.states[0].text(), '失败')
        self.assertTrue(d.rings[0].failed)
        self.assertTrue(d.cancel_button.isDefault())
        d.close()
        d = ImportProgressDialog('demo.dat')
        d.cancelRequested.connect(lambda: signals.append(True))
