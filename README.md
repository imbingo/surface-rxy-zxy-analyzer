...
- XYZ 点表、Pixel XY 与 Z Matrix 的通用格式推断不再覆盖用户选择。
- 八列及更多列的 XYZ 点表可直接进入 XYZ 解析和列映射。
- 默认及复位 3D 视角采用 +X/+Y 对称展开、+Z 向上的工程方向。
- XY、XZ、YZ 投影视图可悬停查看真实源点坐标；处理后的曲面同时显示当前 Z 与原始 Z。
...
- 人工拖动产生的单片层 XY 偏移可直接进入最终容差匹配与胶厚扣减，不要求先执行自动匹配；双层与三层路径均有专项回归。
- 自动匹配改为可选的局部精修，只从当前人工偏移开始，不再与重心初值竞争；自动改变量超过 `max(3×容差, 0.01 mm)` 时拒绝写回并保留人工结果。
- 局部精修只使用当前人工位置附近、距离不超过局部保护半径的“单片→堆叠”候选点，并以固定候选集残差和容差内匹配点数验收；堆叠更大、带孔或只有局部相交时，非重叠区域不会把结果拉偏。全点双向 RMS 仅保留作诊断。
- 单片层记录“未配准 / 人工粗对齐 / 自动精对齐”状态，用于界面、CSV 与报告追溯，不改变实际偏移数学。
- 胶厚报告图新增最终 Inner Gap 点云的 Rx / Ry、平面残差 PV/RMS 和 TTV；全部复用主控页同一 `compute_plane_metrics` 定义。CSV 元数据同步写入 Gap Rx/Ry。
- 胶厚 XY 图支持以鼠标位置为中心的滚轮缩放；拖动、切换单片层、容差刷新、自动精修及结果刷新都保留当前视野，左键双击恢复全图。
- 选择/拖动单片刷新后保持 XY 等比例显示，避免坐标范围不变但点云突然拉伸。
- 胶厚导出使用全部有效匹配点的原始结果快照，主控后续旋转、筛选不改写胶厚导出指标；配准失效后主控导出要求重新计算。
- 平面拟合拒绝共线/退化点集；大点云配准保留全量基准索引、确定性采样求解，并进行全量单片点验收。
- V4.6.3 专项与完整回归共 138 项测试通过。
...
- XY、XZ、YZ、3D 均支持在图内双击恢复各自原始显示状态；3D 同时恢复 X/Y/Z 范围和相机角度，操作不触发重新分析。
...
```powershell
python -m pytest tests/test_gap_registration.py -q
python -m pytest -q
```
