...
try:
    ...
    for width,height in ((1366,768),(1600,900),(1920,1080),(900,640)):
        ...
        for mode in (0,1):
            ...
            app.processEvents()
            w.grab().save(str(out/f'v469_{width}_{mode}.png'))
    fig = w._render_report_figure('长文件名验证_' * 12, x, y, z, w.active_idx,
        w.compute_plane_metrics(x, y, z), 0, '旋转90度 → ' * 12, '关闭', w.import_info)
    fig.savefig(out/'v469_report.png', dpi=120)
    w.resize(1600,900)
    ...
    w.statusBar().clearMessage()
    app.processEvents()
    w.grab().save(str(out/'v469_focused_smallholes.png'))
    from types import SimpleNamespace
    w.on_select(SimpleNamespace(xdata=-12,ydata=-12),SimpleNamespace(xdata=-4,ydata=-9),'XY')
    app.processEvents()
    w.grab().save(str(out/'v469_selection_stable.png'))
    ...
    for mode in (0,1):
        ...
        ready()
        w.grab().save(str(out/f'v469_zoom_{mode}.png'))
