...
echo [ok] Python: %PY%
echo [run] Starting Surface Rxy/Zxy Analyzer V4.5.5
...
:find_python
set "BASE_PY="
if exist "D:\python\python.exe" (set "BASE_PY=D:\python\python.exe"& exit /b 0)
py -3 -c "import sys; print(sys.executable)" > "%TEMP%\surface_analyzer_python_path.txt" 2>nul
if not errorlevel 1 (set /p BASE_PY=<"%TEMP%\surface_analyzer_python_path.txt"& if defined BASE_PY exit /b 0)
